...
if (chartEl) {
  ...
  const x = d3
    .scaleTime()
    .domain([new Date("2020-01-01"), new Date("2025-12-31")]);
  ...
  function zoomed(event) {
    ...
    svg.selectAll(".event-dot").attr("cx", (d) => newX(d.date));
    ...
    if (currentZone !== "all" && groupedMetrics) {
      const countryKey = [...groupedMetrics.keys()].find(
        (k) => k.toLowerCase() === currentZone,
      );
  ...
  async function loadData() {
    try {
      ...
      const uniqueCategories = [
        ...new Set(parsedEvents.map((e) => e.category)),
      ];
      ...
      function applyFilters() {
        ...
        svg
          .selectAll(".event-line, .event-dot")
      ...
      function renderEvents(events) {
        ...
        eventsGroup
          ...
          .attr(
            "y1",
            (d) =>
              height - margin.bottom - (importanceHeight[d.importance] || 10),
          )
        ...
        eventsGroup
          ...
          .attr("r", (d) =>
            d.importance === "high" ? 4 : d.importance === "medium" ? 3 : 2,
          )
        ...
        const parsedMetrics = metrics
          ...
          .filter(
            (d) =>
              ...
              d.date > new Date("2018-12-31"),
          )
          .filter(
            (d) =>
              d.country &&
              uniqueZones.some(
                (z) => z.toLowerCase() === d.country.toLowerCase(),
              ),
          );

        groupedMetrics = d3.group(parsedMetrics, (d) => d.country);

        const allValues = parsedMetrics
          .flatMap((d) => [
